...
- **Profile editor (`/network`) — your-profile-as-first-class-destination** — the TopBar avatar tap routes here (not Settings) when social is enabled. Top-down: completeness gauge → live-preview editor (banner + overlapping avatar with an inline 📷 camera badge that opens the crop dialog directly) → About-you fields (name, skill level, bio, location) → Your links + a **Connect with LinkedIn** CTA below the LinkedIn input (two-mode: real OIDC OAuth when `LINKEDIN_CLIENT_ID` is wired on the social-svc — see [`docs/profile-linkedin.md`](./profile-linkedin.md) — intent-capture fallback when not, runtime-detected so the operator can flip without an SPA redeploy) → **Topics** (the merged Interests + Signals concept; one picker, dual-write to `state.profile.interests` and `social.signals`, max 5; shapes the home feed AND drives discoverability + leaderboards) → People → **Profile visibility at the bottom** (Public / Private) with the 13 field-level toggles tucked inside a closed `<details>` disclosure. The Pronouns field is removed; Settings keeps a thin "✎ Edit my profile" CTA + daily minutes + API key + memory + sign-out, no more duplicate profile editor. Crop dialog is viewport-constrained (action bar always visible above the fold), shorter CTAs ("Save photo" / "Save banner"), explicit –/+ zoom buttons flanking the slider, percentage zoom display. See [`docs/profile.md`](./profile.md) for the senior-PM contract.
- **Privacy Policy + Terms of Use** — public, plain-English legal docs at `/privacy` and `/terms`. Source-of-truth in [`docs/legal/privacy.md`](./legal/privacy.md) + [`docs/legal/terms.md`](./legal/terms.md). The SPA imports them via Vite `?raw` for the in-app view (Legal.tsx); the social-svc sidecar reads them at startup from `/opt/social-svc/legal/` (Dockerfile copies docs/legal/ into the container) and SSRs `/privacy` + `/terms` so LinkedIn's OAuth review crawler, search engines, and Slack/Twitter unfurls get real HTML — not a JS-empty SPA shell. Sign-in card carries an *"By continuing, you agree to our Terms and Privacy Policy."* nudge with hyperlinks; first sign-in stamps an `agreedToLegalAt` audit timestamp on the player identity. Footer with Privacy / Terms / Source links sits site-wide. Required by the LinkedIn OAuth app submission — full operator checklist in [`docs/profile-linkedin.md`](./profile-linkedin.md) §10.
- **LinkedIn Connect (OIDC sign-in) — verified-identity + friction-killer profile fill** — the social-svc exposes `/v1/social/me/linkedin/{config,start,callback}` plus `GET` / `DELETE` for the identity record. Two-bucket storage: **Bucket A (visible + editable)** is a one-time grab on connect (name, photo, email) seeded into `ProfileRecord` — the user owns it from there with a "Use this LinkedIn name" panel offered post-connect. **Bucket B (context + hidden)** captures everything else (`sub`, `emailVerified`, locale, derived `emailDomain` for cold-start "people you may know" matching, `pictureCdnHost`, frozen raw OIDC claims for re-derivation) — immutable by the user, surfaced via a "🔍 What we know about you from LinkedIn" transparency disclosure, used silently to power recommendations and the future verified-human badge. We request *only* `openid profile email` (no `w_member_social` at connect-time, no connections-graph — that LinkedIn API has been closed for years). State tokens are HMAC-signed (5-min TTL, single-use); access tokens are read once and dropped (no persisted bearer). Feature-flagged on `LINKEDIN_CLIENT_ID`: when unset the SPA falls back to v0 intent capture (no SPA redeploy needed to flip). Disconnect deletes both buckets atomically. Full strategy: [`docs/profile-linkedin.md`](./profile-linkedin.md).
