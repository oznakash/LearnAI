...
- Modal dialogs that contain interactive content + an action bar (crop dialog, future create-Spark dialog) MUST constrain the dialog card to the viewport (`max-h-[calc(100dvh-1.5rem)]`) and let the body shrink (`flex-1 min-h-0`) so the action bar is reachable without scrolling on small phones. Header + action bar carry `shrink-0`. Anti-pattern: putting the body on `aspect-ratio: 1/1` with `maxHeight: 70vh` — that lets the body push the action bar past the fold on tall portrait phones.
- Range inputs in dialogs (zoom, opacity) get explicit `−` / `+` buttons flanking the slider when the slider is the primary control. Most users don't pinch; some can't see the slider on a touch device until they realise the slider exists. The buttons make the affordance literal. Pair with a percentage display (`{Math.round(zoom * 100)}%`) instead of `1.23×` — Instagram's mental model wins.
