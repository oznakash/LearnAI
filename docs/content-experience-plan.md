...
- A **creator registry** — `app/src/content/creators.ts` plus `AdminConfig.creators`. Sparks reference creators by `creatorId` (the seed nuggets all use `"lenny"`); the chip name, link URL, link label, and avatar all come from the registry. Adding a new creator (Hard Fork, Stratechery, The Pragmatic Engineer, etc.) is a config change in **Admin → Creators**, not a code change. Sets up the same surface for future source-anchored variants (`VideoNugget`, `EssayNugget`, `NewsletterNugget`).
