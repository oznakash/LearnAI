...
import { parseStalwartQueueId } from "./email-log.js";

// `@types/nodemailer` doesn't declare the (real, supported) RFC 3461
// `dsn` option on Mail.Options. We extend the interface locally so the
// call site stays type-safe without a wholesale `as any` cast.
type DsnNotify = "never" | "success" | "failure" | "delay";
interface DsnOptions {
  id?: string;
  return?: "headers" | "full";
  notify?: DsnNotify | DsnNotify[];
  recipient?: string;
}
type SendMailOptionsWithDsn = Mail.Options & { dsn?: DsnOptions };

// What nodemailer's runtime returns from sendMail. The shipped types
// declare it as `Promise<any>` intersected with `void`, which breaks
// property access on the awaited value. Declared here so we don't
// litter the call site with `as any`.
interface SentInfo {
  messageId?: string;
  response?: string;
  accepted?: unknown[];
...
export interface SendEmailInput {
  ...
  /**
   * RFC 3461 ENVID — opaque identifier we set on `MAIL FROM` so any
   * bounce / delay DSN that arrives at the return-path mailbox can be
   * tied back to the originating log row. We pass our `email-log.ts`
   * row id here. When undefined, DSN is disabled for this message.
   */
  envId?: string;
  /**
   * Return-path mailbox for bounces. Defaults to `SMTP_FROM_EMAIL` so
   * bounces arrive in the same inbox we send from — easy to inspect by
   * hand until the bounce-mailbox poller follow-up lands. Override per
   * call by setting `BOUNCE_EMAIL` env var or passing this field.
   */
...
export interface SendEmailResult {
  ok: boolean;
  /** RFC 5322 Message-ID header value, generated by nodemailer. */
  messageId?: string;
  /**
   * Stalwart's opaque queue id, parsed out of the SMTP 250 response.
   * Use this as the join key for `GET /api/queue/messages/{id}`.
   */
  stalwartQueueId?: string;
  /** Raw SMTP 250 line, kept for forensics. */
  smtpResponse?: string;
  /** Count from `info.accepted` — should be 1 on success. */
  acceptedCount?: number;
  /** Count from `info.rejected` — non-zero means partial failure. */
...
export async function sendEmail(input: SendEmailInput): Promise<SendEmailResult> {
  ...
  // RFC 3461 DSN: request a bounce-style report back to the return-path
  // on FAILURE or DELAY (not SUCCESS — large receivers strip it and it
  // doubles inbound mail volume). Only enabled when envId is set, since
  // without it we can't correlate the inbound DSN back to the log row.
  const bounceRecipient = input.bounceRecipient ?? process.env.BOUNCE_EMAIL ?? cfg.fromEmail;
  const dsn = input.envId
    ? {
        id: input.envId,
        return: "headers" as const,
        notify: ["failure", "delay"] as ("failure" | "delay")[],
        recipient: bounceRecipient,
      }
    : undefined;
  try {
    const opts: SendMailOptionsWithDsn = {
    ...
    const info = (await transporter(cfg).sendMail(opts)) as SentInfo;
    const stalwartQueueId = parseStalwartQueueId(
    ...
    log.info("email_sent", {
      ...
      env_id: input.envId,
      stalwart_queue_id: stalwartQueueId,
    });
    return {
      ok: true,
    ...
    log.error("email_send_failed", { reason: msg, env_id: input.envId });
